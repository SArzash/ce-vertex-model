 ...
 * local_rule.cpp
 *
 * Template for simulating tissue deformation using a local rule for edge-tension
 * dynamics in a vertex model tissue.
 *
 ...
 *   2) Evolve edge tensions with a local update rule
 *       and sample tissue observables.
 *
 * Dependencies:
...
int main(int argc, char *argv[])
{
    ...
    for (int p0OuterIter = 0; p0OuterIter < 1; ++p0OuterIter)
    {
        ...
        // =========================================
        // 5e) Local edge-tension dynamics protocol
        // =========================================
        ...
        for (int tensionIter = 0; tensionIter < tensionDynamicsSteps; ++tensionIter)
        {
            ...
            avm->EdgeTensionLocalRule(fireDt, tensionDynamicsDamping);
